    ...
    <SafeAreaView style={g.screen}>
      ...
      <ScrollView contentContainerStyle={g.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={g.cardList}>
          {keys.map((key, idx) => {
            if (idx === 0) console.log('keys:', keys, 'day:', day?.id, 'ca keys:', Object.keys(ca));
            ...
            if (key === 'challenge') console.log('challenge:', { quizDone, isLocked, isDone: !!ca[`${day.id}_${key}`], idx, doneCount });
            ...
            return (
              <ActivityCard
                ...
                activity={acts[key]}
                completed={isDone}
                isActive={!isDone && !isLocked && (isFree || (!isFree && idx === doneCount))}
                isLocked={isLocked}
                onPress={() => open(key)}
              />
            );
